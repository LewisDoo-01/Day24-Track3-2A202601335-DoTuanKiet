...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Specifying target entities prevents false positives on names/dates in Vietnamese
...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    refuse_keywords = ["xin lỗi", "không thể", "không được phép", "i cannot", "i'm sorry", "truy cập bị từ chối"]
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    refuse_keywords = ["xin lỗi", "không thể cung cấp", "i cannot", "truy cập bị từ chối"]
...
if __name__ == "__main__":
    ...
    print("============================================================")
    print("PHASE C: Production Guardrails Evaluation")
    print("============================================================")

    print("Initializing guardrail engines (Presidio + NeMo)...")
    analyzer, anonymizer = setup_presidio()
    rails = setup_nemo_rails()
    print("✓ Guardrail engines ready")

    # Task 9a: PII scan demo
    print("\n[Demo] Running PII scan...")
    ...
    print(f"\nRunning adversarial suite on {len(adversarial_set)} inputs...")
    results = run_adversarial_suite(adversarial_set, rails, analyzer, anonymizer)
    
    passed_count = sum(1 for r in results if r["passed"])
    pass_rate = passed_count / len(results)
    print(f"✓ Adversarial suite finished: {passed_count}/{len(results)} passed ({pass_rate:.1%})")

    # Task 12: P95 latency
    print("\nMeasuring P95 latency (10 runs)...")
    ...
    print(f"✓ Latency measurement done:")
    print(f"  Presidio P95: {latency['presidio_ms']['p95']:.1f}ms")
    print(f"  NeMo P95:     {latency['nemo_ms']['p95']:.1f}ms")
    print(f"  Total P95:    {latency['total_ms']['p95']:.1f}ms")
    print(f"  Latency budget check: {'PASS' if latency['latency_budget_ok'] else 'FAIL'} (Budget: {latency['budget_ms']}ms)")
    ...
    print("\n✓ Saved Phase C results → reports/guard_results.json")
